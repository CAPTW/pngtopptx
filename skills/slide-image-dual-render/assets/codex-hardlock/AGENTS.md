...
- Collect original font names before rendering and search both system and user font locations.
- Never install a missing font automatically. Stop before rendering, emit the install request, and ask the user.
- After the user declines installation or installation is unavailable, apply a fallback, record every `Original -> Resolved` mapping, and continue conversion.
...
If the gate fails, the output is not deliverable. Fix the slide fragments, crop plan, font decision/mapping, or renderer contract and rerun `slide_pipeline.js`.

Font preflight writes `out/font_install_request.json` and `out/font_resolution_manifest.json`.
The default decision is `ask`. A missing font must be presented to the user before any installation.
Use `--font-install-decision declined` or `--font-install-decision unavailable` only after that user
decision; both modes apply a documented fallback and continue conversion. `final_gate.js` rejects a
deck whose mapping is incomplete or claims that the resolver attempted automatic installation.
