...
## Original-font resolution and PPTX/HTML parity policy

Before rendering, collect every known original font from the profile, literal `fontFace`/`fontFamily`
values in `lib/slides.js`, and `work/font_usage.json` (including per-slide usage manifests). Search both
system-wide and per-user font locations and the Windows HKLM/HKCU font registries. An installed
...
- HTML → a real browser uses the same locally resolved font as PPTX and runs the shrink-to-fit script. `wkhtmltoimage`
