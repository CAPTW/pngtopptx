...
Generate a Codex-ready prompt for a repair wave.
...
function buildPrompt(opts, wave) {
  ...
  return `Use [$slide-image-dual-render](C:\\Users\\USER\\.codex\\skills\\slide-image-dual-render\\SKILL.md) and [$slide-visual-polish-qa](C:\\Users\\USER\\.codex\\skills\\slide-visual-polish-qa\\SKILL.md).
...
node "$env:USERPROFILE\\.codex\\skills\\slide-image-dual-render\\scripts\\slide_pipeline.js" --project . --slides ${slidesArg} --quality reconstruction --require-qa --require-reconstruction --crop-plan work\\crop_plan.json --node-path .\\node_modules --target both --pptx-out out\\${stem}.pptx --html-out out\\${stem}.html

node "$env:USERPROFILE\\.codex\\skills\\slide-image-dual-render\\scripts\\final_gate.js" --project . --slides ${slidesArg} --quality reconstruction --require-qa --require-reconstruction --require-pptx-openable --target both --pptx out\\${stem}.pptx --html out\\${stem}.html

python "$env:USERPROFILE\\.codex\\skills\\slide-visual-polish-qa\\scripts\\rasterize_pptx.py" --project . --pptx out\\${stem}.pptx --source-slides ${slidesArg} --out-dir work

python "$env:USERPROFILE\\.codex\\skills\\slide-visual-polish-qa\\scripts\\capture_html_screenshot.py" --project . --html out\\${stem}.html --source-slides ${slidesArg} --out-dir work --width 1672 --height 941

python "$env:USERPROFILE\\.codex\\skills\\slide-visual-polish-qa\\scripts\\compare_slide_images.py" --project . --slides ${slidesArg} --mode qa-polish --source-dir src --qa-dir work --out-summary out\\visual_qa_summary_${stem}.json

node "$env:USERPROFILE\\.codex\\skills\\slide-visual-polish-qa\\scripts\\generate_visual_qa_summary.js" --project . --slides ${slidesArg} --out-json out\\visual_qa_summary_${stem}.json --out-md out\\visual_qa_summary_${stem}.md

node "$env:USERPROFILE\\.codex\\skills\\slide-visual-polish-qa\\scripts\\enforce_visual_qa.js" --project . --slides ${slidesArg} --mode qa-polish --summary out\\visual_qa_summary_${stem}.json --require-pptx --require-html
